...
- **禁用被弃用的 flag API**。`lv_obj_add_flag` / `lv_obj_remove_flag` / `lv_obj_has_flag` / `lv_obj_has_flag_any` 在本版 LVGL 9.6-dev 中带 `__attribute__((deprecated))`（见 `lvgl/include/lvgl/core/lv_obj.h:234,243,540,550`），`-Werror` 下会编译失败。一律改用 per-flag 专用读写函数：

  | 弃用写法 | 必须改成 |
  |---|---|
  | `lv_obj_add_flag(o, LV_OBJ_FLAG_HIDDEN)` | `lv_obj_set_hidden(o, true)` |
  | `lv_obj_remove_flag(o, LV_OBJ_FLAG_HIDDEN)` | `lv_obj_set_hidden(o, false)` |
  | `lv_obj_add_flag(o, LV_OBJ_FLAG_CLICKABLE)` | `lv_obj_set_clickable(o, true)` |
  | `lv_obj_remove_flag(o, LV_OBJ_FLAG_CLICKABLE)` | `lv_obj_set_clickable(o, false)` |
  | `lv_obj_remove_flag(o, LV_OBJ_FLAG_SCROLLABLE)` | `lv_obj_set_scrollable(o, false)` |
  | `lv_obj_has_flag(o, LV_OBJ_FLAG_HIDDEN)` | `lv_obj_is_hidden(o)` |
...
在 `lv_hex_menu_create()` 中，`lv_obj_set_scrollable(obj, false);` 之后补一行：
